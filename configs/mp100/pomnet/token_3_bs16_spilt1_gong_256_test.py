...
evaluation = dict(interval=1, metric='PCK', key_indicator='PCK', gpu_collect=True)
optimizer = dict(
    type='Adam',
    lr=5e-4,
    paramwise_cfg=dict(
        custom_keys={'encoder_sample': dict(lr_mult=0)})
)
...
valid_pipeline = [
    ...
    dict(
        ...
        keys=['img', 'target', 'target_weight'],
        meta_keys=[
            'image_file', 'joints_3d','center', 'scale', 'rotation', 'bbox_score',
