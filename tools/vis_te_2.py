...
# 载入数据
df = pd.read_csv(r'/data/yjliang/code/Category-Agnostic-Pose-Estimation/P2/Pose-for-Everything/work_dirs/token_3_bs16_spilt1_gong_256/test/test.csv')
...
def plot_embedding(data, label, title):
	...
	for i in range(data.shape[0]):
		# 在图中为每个数据点画出标签
		if int(label[i])<21:
		#if label[i]==8 or label[i]==12 or label[i]==17 or label[i]==18 or label[i]==5 or label[i]==6 or label[i]==7:
