...
class TransformerPose(BasePose):
    ...
    def parse_keypoints_from_img_meta(self, img_meta, device):
        ...
        """
        query_kpt = torch.stack([torch.tensor(info['query_joints_3d']).to(device) for info in img_meta], dim=0)[:,:,:2] # [bs, num_query, 2]
        return query_kpt
    ...
    def forward_train(self, img_s, target_s, target_weight_s,
        ...
        feature_q = self.encoder_sample(img_q)

        mask_s = target_weight_s[0]
        for target_weight in target_weight_s:
            mask_s = mask_s * target_weight

        target_keypoints = self.parse_keypoints_from_img_meta(img_metas, feature_q.device)
        point_list,output,similary = self.keypoint_head(feature_q, feature_s, target_s, mask_s,img_metas)
        point_list.append(output)
        # if return loss
        losses = dict()
        target_sizes = torch.tensor([img_q.shape[-2], img_q.shape[-1]]).unsqueeze(0).repeat(img_q.shape[0], 1, 1)
        if self.with_keypoint:
            keypoint_losses = self.keypoint_head.get_loss(
                point_list,similary, target_keypoints,target_q, target_weight_q * mask_s,target_sizes)
            losses.update(keypoint_losses)
            keypoint_accuracy = self.keypoint_head.get_accuracy(
                output, target_keypoints, target_weight_q * mask_s,target_sizes)
