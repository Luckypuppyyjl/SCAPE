...
from .post_transforms import (affine_transform,
                              get_affine_transform,fliplr_joints)
...
class TopDownAffineFewShot:
    ...
    def __call__(self, results):
        ...
        s = results['scale']
        r = results['rotation']
        if results['flip']==True and np.random.rand() <= 0.5 and results['pair']!=None :
            img = img[:, ::-1, :]
            joints_3d, joints_3d_visible = fliplr_joints(joints_3d, joints_3d_visible, img.shape[1], results['pair'])
